...
async def consultation_callback(callback: CallbackQuery):
    await edit(
        ...
        "Сначала я отправлю вам небольшую анкету о питомце и его поведении. "
        "Постарайтесь ответить на вопросы максимально подробно — это поможет мне "
        "заранее познакомиться с вашей ситуацией.\n\n"
        "После заполнения анкеты мы договоримся об удобном времени. "
        "Консультация длится 60 минут и проходит по видеосвязи или, если вам удобнее, "
        "в аудиоформате.\n\n"
        "На встрече мы подробно разберём вашу ситуацию, причины поведения кошки "
        "и возможные способы его коррекции.\n\n"
        "После консультации я отправлю вам письменные рекомендации и план дальнейших "
        "действий, чтобы вся информация осталась у вас под рукой. Также в стоимость "
        "входит поддержка в мессенджере в течение недели — можно будет задать вопросы, "
        "которые появятся после встречи.",
        keyboard([
